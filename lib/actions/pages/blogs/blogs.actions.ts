...
import { logBlogsActivity } from "./shared";
...
export const createBlogAction = async (
  ...
  try {
    const existingBlog = await (async () => {
      try {
        const blog = await (
          await import("../../../db/repository/pages/blogs/blog.service")
    ...
    if (newBlog) {
      await logBlogsActivity("New blog post added", user.name, user.role);
...
export const deleteBlogAction = async (id: string) => {
  ...
  try {
    ...
    if (deleted) {
      await logBlogsActivity("Blog post deleted", user.name, user.role);
